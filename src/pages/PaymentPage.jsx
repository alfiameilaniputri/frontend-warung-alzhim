...
//   const { transactionId } = useParams(); // ambil orderId dari route
...
//         const res = await fetch(`${API_URL}/api/order/${transactionId}`, {
//           headers: {
...
export default function PaymentPage() {
  const navigate = useNavigate();
  const { orderId } = useParams(); // Ambil orderId dari route params
  const [order, setOrder] = useState(null);
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      try {
        ...
        // FIX: Endpoint yang benar sesuai backend
  ...
  useEffect(() => {
    const handleMessage = (event) => {
      // Hanya terima message dari Midtrans
